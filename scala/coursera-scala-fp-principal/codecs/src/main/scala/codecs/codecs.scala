...
trait DecoderInstances:
  ...
  given Decoder[Int] = 
    Decoder.fromPartialFunction{ case Json.Num(b) if b.isValidInt => b.intValue }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
      Decoder.fromFunction {
          case Json.Obj(m) => m.get(name).flatMap(decoder.decode)
          case _ => Option.empty
      }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person]{case (name, age) => Person(name, age)} //question same as ((name, age)=>Person(name, age)) ?
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] = Decoder.field[List[Person]]("people").transform(Contacts(_))
