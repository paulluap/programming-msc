...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = 
    //why not working if we do right.filterAcc first then left
    val subtree = right.filterAcc(p, left.filterAcc(p, acc))
    if p(elem) then
      val r = subtree.incl(elem)
      r
    else
      subtree
  ...
  def mostRetweeted: Tweet = 
    def larger(t1: Tweet, t2: Tweet): Tweet = 
    ...
    if left.isEmpty && right.isEmpty then 
      ...
      larger(elem, right.mostRetweeted)
    else if right.isEmpty then
      larger(elem, left.mostRetweeted)
    else
      larger(elem, larger(left.mostRetweeted, right.mostRetweeted))

  def descendingByRetweet: TweetList = 
    val tweet = mostRetweeted
    Cons(tweet, remove(tweet).descendingByRetweet) 
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = 
    TweetReader.allTweets.filter(t=>google.exists(e => t.text.contains(e)))

  lazy val appleTweets: TweetSet = 
    TweetReader.allTweets.filter(t=>apple.exists(e => t.text.contains(e)))
