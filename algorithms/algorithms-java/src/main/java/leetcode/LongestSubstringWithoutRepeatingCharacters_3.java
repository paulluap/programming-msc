...
public class LongestSubstringWithoutRepeatingCharacters_3 {
    ...
    public static int lengthOfLongestSubstring(String s) {
        ...
        int ll =0, l=0, chopOff=0;
        for(int i=0; i<s.length(); i++){ //i is useless..
            char c = s.charAt(i);
            System.out.println(i + ", last chopoff: " + chopOff + " current chopoff: " + indexMap.get(c));
            if (indexMap.get(c) != null && (chopOff == 0 || indexMap.get(c) > chopOff)/*ignore what we have chopped off*/) {
